...
#if 0
static TRUTH_VALUE_T mget_wapi_key_ok(void *priv)
{
    struct lib80211_wpi_data *data = priv;
    return data->wapi_key_ok;
}
#endif
static void mset_wapi_key_ok(TRUTH_VALUE_T val, void *priv)
{
    struct lib80211_wpi_data *data = priv;
    data->wapi_key_ok = val;
}
#if 0
static TRUTH_VALUE_T mget_wapi_enable(void *priv)
{
    struct lib80211_wpi_data *data = priv;
    return data->wapi_enable;
}
static void mset_wapi_enable(TRUTH_VALUE_T val, void *priv)
{
    struct lib80211_wpi_data *data = priv;
...
#endif
#if 0
static u8* mget_wapi_version(void *priv)
{
    struct lib80211_wpi_data *data = priv;
    return data->wapi_version;
}
static void mset_wapi_version(u8* val, void *priv)
{
    struct lib80211_wpi_data *data = priv;
...
#endif
#if 0
static u8* mget_wapi_address(void *priv)
{
    struct lib80211_wpi_data *data = priv;
    return data->ap_address;
}
static void mset_wapi_address(u8* val, void *priv)
{
    struct lib80211_wpi_data *data = priv;
    memcpy(data->ap_address,val, ETH_ALEN);
}
#endif
static u8 mget_key_index(void *priv)
{
    struct lib80211_wpi_data *data = priv;
    return data->key_index;
}
static void mset_key_index(int index, void *priv)
{
    struct lib80211_wpi_data *data = priv;
    if(index <= 3 ) {
...
#if 0
static u8* mget_pn_key(void *priv)
{
    struct lib80211_wpi_data *data = priv;
    return data->pn_key;
}
#endif
static void mset_pn_key(u8* val, void *priv)
{
    struct lib80211_wpi_data *data = priv;
    memcpy(data->pn_key,val,WAPI_PN_LEN);
}
static u8 *inc_pn_key(void *priv)
{
    struct lib80211_wpi_data *data = priv;
    int i;
    data->pn_key[15] += 2;
...
static u8 *mget_pmsk_key(int index, void *priv)
{
    struct lib80211_wpi_data *data = priv;
    return ( index >= 3 ) ? NULL : data->pmsk_key[index];
}
static void mset_pmsk_key(int index,u8* val, void *priv)
{
    struct lib80211_wpi_data *data = priv;
    if(index < 3 ) {
...
static u8* mget_mic_key(int index, void *priv)
{
    struct lib80211_wpi_data *data = priv;
    return ( index >= 3 ) ? NULL : data->mic_key[index];
}
static void mset_mic_key(int index,u8* val, void *priv)
{
    struct lib80211_wpi_data *data = priv;
    if(index < 3 ) {
...
static u16 wlan_tx_wapi_encryption(u8 * header,
...
static BOOL_T is_group(u8* addr)
{
    if((addr[0] & BIT(0)) != 0)
        return BTRUE;
    return BFALSE;
}
static u16 wlan_rx_wapi_decryption(u8 * input_ptk,u16 header_len,u16 data_len,
