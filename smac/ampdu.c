...
static int __maybe_unused ssv6200_get_ampdu_max_transmit_length(int rate_idx)
{
    ...
    struct AMPDU_TID_st *cur_AMPDU_TID)
{
    ...
    // Attempting to fix ht_cap access for Kernel 6.12 (MLO)
    // Assuming 'deflink' is the correct path for non-MLO or default link access
...
static void __maybe_unused _flush_mpdu (struct ssv_softc *sc, struct ieee80211_sta *sta)
{
    unsigned long flags;
    struct ssv_sta_priv_data *ssv_sta_priv = (struct ssv_sta_priv_data *) sta->drv_priv;
    int i;
...
static int __maybe_unused _dump_ba_skb (struct ssv_softc *sc, char *buf, int buf_size, struct sk_buff *ba_skb)
{
