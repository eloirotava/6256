...
static inline void lib80211_ccmp_aes_encrypt(struct crypto_sync_skcipher *tfm,
        const u8 pt[16], u8 ct[16])
{
    ...
    memcpy(ct, pt, 16);
    sg_init_one(sg, ct, 16);
    skcipher_request_set_sync_tfm(req, tfm);
    skcipher_request_set_callback(req, 0, NULL, NULL);
    skcipher_request_set_crypt(req, sg, sg, 16, NULL);
    
    if (crypto_skcipher_encrypt(req)) {
        printk(KERN_ERR "ccmp: aes encryption failed\n");
    }
}
// wait, sg_init_one takes a pointer. If I point sg to ct, and copy pt to ct first.
...
static void *lib80211_ccmp_init(int key_idx)
{
    ...
    priv->tfm = crypto_alloc_sync_skcipher("ecb(aes)", 0, 0);
    if (IS_ERR(priv->tfm)) {
        ...
        printk(KERN_ERR "Found %s in driver %s (M %s).\n",
               ...
               "kernel"); 
               // Accessing internal members of tfm is unsafe/hard. Simplified print.
    }
#ifdef MULTI_THREAD_ENCRYPT
...
fail:
    if (priv) {
        if (priv->tfm)
            crypto_free_sync_skcipher(priv->tfm);
#ifdef MULTI_THREAD_ENCRYPT
        if(priv->tx_b0 != NULL)
            kfree(priv->tx_b0);
        if(priv->tx_b != NULL)
...
static void lib80211_ccmp_deinit(void *priv)
{
    ...
    if (_priv && _priv->tfm)
        crypto_free_sync_skcipher(_priv->tfm);
#ifdef MULTI_THREAD_ENCRYPT
    if(_priv->tx_b0 != NULL)
        kfree(_priv->tx_b0);
    if(_priv->tx_b != NULL)
...
static void ccmp_init_blocks(struct crypto_sync_skcipher *tfm,
...
static int lib80211_ccmp_set_key(void *key, int len, u8 * seq, void *priv)
{
    ...
    int keyidx;
    struct crypto_sync_skcipher *tfm = data->tfm;
#ifdef MULTI_THREAD_ENCRYPT
    u8 *tx_b0 = data->tx_b0;
    u8 *tx_b = data->tx_b;
    u8 *tx_e = data->tx_e;
    ...
    if (len == CCMP_TK_LEN) {
        ...
        crypto_sync_skcipher_setkey(data->tfm, data->key, CCMP_TK_LEN);
#ifdef MULTI_THREAD_ENCRYPT
        data->tx_b0 = tx_b0;
        data->tx_b = tx_b;
        data->tx_e = tx_e;
