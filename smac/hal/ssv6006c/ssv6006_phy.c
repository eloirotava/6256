...
static void ssv6006_fill_lpbk_tx_desc(struct sk_buff *skb, int security, unsigned char rate)
{
    struct ssv6006_tx_desc *tx_desc;
...
static void ssv6006_rc_mac80211_rate_idx(struct ssv_softc *sc,
...
static void _update_green_tx(struct ssv_softc *sc, u16 rssi)
{
...
static void ssv6006_update_scan_cci_setting(struct ssv_softc *sc)
{
    if (sc->cci_set) {
...
static void ssv6006_recover_scan_cci_setting(struct ssv_softc *sc)
{
...
static void ssv6006_update_data_cci_setting
...
#endif
static void ssv6006_update_rxstatus(struct ssv_softc *sc,
...
static bool ssv6006_is_legacy_rate(struct ssv_softc *sc, struct sk_buff *skb)
{
    bool ret = true;
...
static void ssv6006_rc_legacy_bitrate_to_rate_desc(int bitrate, u8 *drate)
{
    *drate = 0;
    switch (bitrate) {
    case 10:
...
static s32 ssv6006_rc_ht_update_rate(struct sk_buff *skb, struct ssv_softc *sc, struct fw_rc_retry_params *ar)
{
    return ssv_minstrel_ht_update_rate(sc, skb);
}
static bool ssv6006_rc_ht_sta_current_rate_is_cck(struct ieee80211_sta *sta)
{
    return ssv_minstrel_ht_sta_is_cck_rates(sta);
}
#define NUMBER_OF_MCS 8
static int ssv6006_ampdu_max_transmit_length(struct ssv_softc *sc, struct sk_buff *skb, int rate_idx)
{
    struct ieee80211_tx_info *info = IEEE80211_SKB_CB(skb);
