...
#endif
#ifdef CONFIG_HAS_WAKELOCK
struct wakeup_source *icomm_wake_lock;
#endif
...
{
    __pm_wakeup_event(icomm_wake_lock, jiffies_to_msecs(HZ));
    printk("***** %s ******\n", __func__);
    return IRQ_HANDLED;
}
...
static int wifi_resume(struct platform_device *pdev)
{
    ...
    __pm_wakeup_event(icomm_wake_lock, 5000);
    return 0;
}
...
int initWlan(void)
{
    int ret = 0;
#ifdef CONFIG_HAS_WAKELOCK
    icomm_wake_lock = wakeup_source_register(NULL, "ssv6051");
    __pm_stay_awake(icomm_wake_lock);
#endif
...
#ifdef CONFIG_HAS_WAKELOCK
    wakeup_source_unregister(icomm_wake_lock);
#endif
    return;
}
