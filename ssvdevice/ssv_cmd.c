...
void ssv6xxx_send_noa_cmd(struct ssv_softc *sc, struct ssv6xxx_p2p_noa_param *p2p_noa_param)
{
    ...
    memcpy((void *)host_cmd->dat32, p2p_noa_param, sizeof(struct ssv6xxx_p2p_noa_param));
...
static int ssv_cmd_iqk (struct ssv_softc *sc, int argc, char *argv[])
{
    ...
    }
    {
        ...
        memcpy((void *)host_cmd->dat32, &cmd_iqk_cfg, IQK_CFG_LEN);
        memcpy((u8 *)host_cmd->dat8 + IQK_CFG_LEN, phy_tbl, phy_setting_size);
        memcpy((u8 *)host_cmd->dat8 + IQK_CFG_LEN + phy_setting_size, rf_tbl, rf_setting_size);
...
static int ssv_cmd_rxtput(struct ssv_softc *sc, int argc, char *argv[])
{
    ...
    host_cmd->len = skb->data_len;
    memcpy((void *)host_cmd->dat32, &cmd_rxtput_cfg, sizeof(struct sdio_rxtput_cfg));
    if (sc->sh->hci.hci_ops->hci_send_cmd(sc->sh->hci.hci_ctrl, skb) == 0) {
...
struct ssv_cmd_table cmd_table[] = {
    ...
    { "cali", ssv_cmd_cali, "calibration for ssv6x5x", 4096},
    { "init", ssv_cmd_init, "re-init ", 64},
    { "rc", ssv_cmd_rc, "fix rate set for ssv6x5x", 4096},
    { "lpbk", ssv_cmd_lpbk, "lpbk test for ssv6x5x", 4096},
    { "hwmsg", ssv_cmd_hwinfo, "hw message for ssv6x5x", 4096},
